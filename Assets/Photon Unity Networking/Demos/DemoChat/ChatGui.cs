...
public class ChatGui : MonoBehaviour, IChatClientListener
{
...
#endif
        userChannelName = UserDetailsManager.userName.ToLower();
        Debug.Log("Channel Name: " + userChannelName);
        this.chatClient.Connect(UserDetailsManager.PhotonChatID, "1.0", new Photon.Chat.AuthenticationValues(userChannelName));
    ...
    public void sendPhotonNotification(string toplayerName, string fromplayername, string msg)
    {
        string receiver = toplayerName.ToLower();
        //Debug.Log("ToPlayer" + receiver);
        
            chatClient.SendPrivateMessage(receiver, msg+";"+fromplayername);
    ...
    public void OnPrivateMessage(string sender, object message, string channelName)
    {
       ...
       /* if (this.selectedChannelName.Equals(channelName))
        ...
        Debug.Log(sender + " : notificationAYA"+"/"+ message);

        if (sender != userChannelName)
        {
            if (message.ToString().Contains("Join_Room"))
            {
                if (message.ToString().Split(';')[1].Equals(UserDetailsManager.userName))
                {
                    string msg = message.ToString().Split(';')[2];
                    notification.SetActive(true);
                    notification.transform.GetChild(0).GetComponent<Text>().text = msg;
                    Debug.Log(msg);
                    StartCoroutine(hideSearchPanel());
                }
            }
            
            else if (message.ToString().Contains("sendChallenge"))
            {
                Debug.Log("Received Challenge");
                notification.SetActive(true);
                int bet = int.Parse(message.ToString().Split(';')[2]);
                int winAmt = int.Parse(message.ToString().Split(';')[3]);
                string game = message.ToString().Split(';')[4];
                if (game.Contains("Whot"))
                {
                    notification.GetComponent<ChallengeManager>().poolId = message.ToString().Split(';')[1];
                    notification.GetComponent<ChallengeManager>().sender = sender;
                    notification.GetComponent<ChallengeManager>().betAmount = bet;
                    notification.GetComponent<ChallengeManager>().winAmount = winAmt;
                    string msg = sender + " has challenged you for bet of " + bet + " coins in Whot Game!!";
                    notification.transform.GetChild(0).transform.GetChild(0).GetComponent<Text>().text = msg;
                    Debug.Log(msg);
                }
                else if(game.Contains("Ludo"))
                {
                    notification.GetComponent<LudoChallengeManager>().poolId = message.ToString().Split(';')[1];
                    notification.GetComponent<LudoChallengeManager>().sender = sender;
                    notification.GetComponent<LudoChallengeManager>().betAmount = bet;
                    notification.GetComponent<LudoChallengeManager>().winAmount = winAmt;
                    string msg = sender + " has challenged you for bet of " + bet + " coins in Ludo Game!!";
                    notification.transform.GetChild(0).transform.GetChild(0).GetComponent<Text>().text = msg;
                    Debug.Log(msg);
                }
            }
            else if (message.ToString().Contains("accepted"))
            {
                Debug.Log("Challenge Accepted!");
                if (message.ToString().Contains("Whot"))
                    WHOTMultiplayerManager.Instance.StopHideNotification();
                else if (message.ToString().Contains("Ludo"))
                    LudoMultiplayer.Instance.StopHideNotification();
            }
            else if (message.ToString().Contains("declined"))
            {
                Debug.Log("Challenge Declined!");
                if (message.ToString().Contains("Whot"))
                {
                    WHOTMultiplayerManager.Instance.loadingText.text = "Opponent Not Available!";
                    WHOTMultiplayerManager.Instance.isChallenge = false;
                    StartCoroutine(WHOTMultiplayerManager.Instance.DeletePool());
                ...
                {
                    //LudoMultiplayer.Instance.loadingText.text = "Opponent Not Available!";
                    LudoMultiplayer.Instance.isChallenge = false;
                    StartCoroutine(LudoMultiplayer.Instance.DeletePool());
                    PhotonNetwork.LeaveRoom();
                    LudoMultiplayer.Instance.StopHideNotification();
                    StartCoroutine(hideSearchPanel());
                }
	...
	IEnumerator hideSearchPanel()
    {
        if (SceneManager.GetActiveScene().name == "LudoMenu")
        {
            LudoMultiplayer.Instance.loadingText.text = "Challenge Declined!!";
        }
            yield return new WaitForSeconds(2f);
        if (SceneManager.GetActiveScene().name == "WhotPlay")
            WHOTMultiplayerManager.Instance.SearchOpponentPanel.SetActive(false);
        else if (SceneManager.GetActiveScene().name == "LudoMenu")
        {
            LudoMultiplayer.Instance.SearchOpponentPanel.SetActive(false);
        }
    }
    /// <summary>
