...
public class Board : MonoBehaviour {
    ...
    public List<GameObject> HighLightedArray;
    /**
     * Private Variables.
     */
    ...
    public ArrayList allPlayerPieces;
    private ArrayList allEnemyPieces;
    public ArrayList paintedPositions = null;
    ...
    void Awake()
    {
        ...
        if(this.GetComponent<Image>().color==redLineColor)
        {
          Debug.LogError("colorfull names "+  this.GetComponent<Image>().name);
        }
    ...
    private IEnumerator FinishMovement()
    {
        ...
        {
            gameController.NotifyPlayerEndOfMovementBot();
        }
    ...
    public void TileClickedOther(TileHandler tile)
    {
        // Set that this event was called.
        this.tileClickedCalled = true;
        // Remove selections.
        ...
        this.gameController.SendOtherToPlayer(tile);
    ...
    public void DeselectTiles()
    {
        ...
        if (paintedPositions != null)
        {
            if (CheckersMultiplayer.Instance.IsTableTen)
            {
                ...
                {
                    if (this.GetTile(tilePosition).GetComponent<TileHandler>().IsRed)
                    {
                        this.GetTile(tilePosition).GetComponent<Image>().color = redLineColor;
                    }
                    else
                    {
    ...
    public void MovePiece( Movement move )
    {
        ...
        if(this.GetTile(move.getOriginalPosition())
            .transform.GetChild(0).GetComponent<Piece>() !=null)
        {
    ...
    public void ResetPossibleMovements(ArrayList possibleMoves)
    {
        if (possibleMoves != null)
        {
            ...
            {
                if (GetTile(e.getDestinyPosition())
                    .GetComponent<TileHandler>().IsRed)
                {
                    GetTile(e.getDestinyPosition())
                        .GetComponent<Image>().color = redLineColor;
                }
                else
                {
                    GetTile(e.getDestinyPosition())
                       .GetComponent<Image>().color = blackTileColor;
                }
            }
        }
    }
    ...
    public bool SomePieceCanCaptureE()
    {
        ArrayList captureMovementsE;
        foreach (Piece piece in allEnemyPieces)
        {
            captureMovementsE = piece.GetCaptureMovements();
            if (captureMovementsE.Count != 0)
            {
                return true;
            }
        }
    ...
    public void DestroyCapturedPieces()
    {
        ...
        foreach (Piece piece in allEnemyPieces)
        {
           // Debug.LogError("enemy pieces " + piece);
