...
namespace Ludo
{
    public class ControlAvatars : MonoBehaviour
    {
       ...
       // private float leftCoinsPos = -500;
        ...
        {
            LudoMultiplayer.Instance.DisconnectFromPhoton();
        ...
        public void reset()
        {
            ...
            GameManager.Instance.requiredPlayers = LudoMultiplayer.Instance.playerCount;
            if(GameManager.Instance.requiredPlayers == 4)
            {
            ...
            Debug.Log("StaticStrings.isFourPlayerModeEnabled: "+ StaticStrings.isFourPlayerModeEnabled);
            if (!StaticStrings.isFourPlayerModeEnabled)
            {                
                for (int i = 1; i < OppoProgressBar.Length; i++)
                {
                    Debug.Log("Disabling Opponent: "+ i);
                    OppoAvatar[i].SetActive(false);
                    InviteToJoinButtons[i].SetActive(false);
            ...
            {
                for (int i = 0; i < LudoMultiplayer.Instance.hideObj.Length; i++)
                {
                    LudoMultiplayer.Instance.hideObj[i].SetActive(false);
                }
            ...
            else
            {
                for (int i = 0; i < LudoMultiplayer.Instance.hideObj.Length; i++)
                {
                    LudoMultiplayer.Instance.hideObj[i].SetActive(true);
                }
        ...
        public void ShareCode()
        {
            ...
            string shareText = StaticStrings.SharePrivateLinkMessage + " "  + StaticStrings.SharePrivateLinkMessage2 + " "; //Removed RoomID
#if UNITY_ANDROID
        ...
        public void PlayerJoined(int index, string id)
        {
            ...
            if (GameManager.Instance.opponentsIDs.Contains(id))
            {
                ...
                if (GameManager.Instance.opponentsAvatars[index] != null)
                    OppoAvatarImage[index].GetComponent<RawImage>().texture = LudoMultiplayer.Instance.defaultPlayerAvatar;

                Debug.Log("Offline Mode: " + GameManager.Instance.offlineMode);
                ...
                if (GameManager.Instance.offlineMode)
                {
                    ...
                    if (GameManager.Instance.currentPlayersCount >= GameManager.Instance.requiredPlayers)
                    {
                        ...
                        LudoMultiplayer.Instance.StartGame();
                    }
                    else
                    {
                        LudoMultiplayer.Instance.WaitForNewPlayer();
                ...
                if (foundPlayer)
                {
                    ...
                    if (speed1 < speed * (2.8f / 4.0f) && !changedAvatar)
                    {
                        ...
                        if (GameManager.Instance.avatarOpponent != null)
                            lastAvatar.GetComponent<RawImage>().texture = GameManager.Instance.avatarOpponent.texture;
                        else
                            lastAvatar.GetComponent<RawImage>().texture = noAvatarSprite.texture;
                    }
        ...
        */
        ...
        {
            LudoMultiplayer.Instance.imReady = true;
            if (!GameManager.Instance.offlineMode)
            {
        ...
        public void cancelGameInvoke()
        {
            ...
            if (!foundPlayer && PhotonNetwork.otherPlayers.Length == 0)
            {
                ...
                PhotonNetwork.BackgroundTimeout = StaticStrings.photonDisconnectTimeoutLong; ;
                Debug.Log("Timeout 2");
                LudoMultiplayer.Instance.imReady = false;
            }
        }
