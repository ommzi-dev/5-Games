...
public class LockZPosition : MonoBehaviour {
    ...
    void OnTriggerEnter(Collider other) {
        ...
        if (!poted && other.tag.Contains("Pot")) {
            audioSources[1].Play();
            PoolGameManager.Instance.ballTouchedBand++;
            ...
            if (!firstBallPot) {
                ...
                if (transform.tag.Equals("WhiteBall")) {
                    ...
                    if (ballNumber == 8) {
                        ...
                        PoolGameManager.Instance.iLost = true;
                    } else if (ballNumber < 8) {
                        firstBallPot = true;
                        PoolGameManager.Instance.noTypesPotedSolid = true;
                    } else if (ballNumber > 8) {
                        firstBallPot = true;
                        PoolGameManager.Instance.noTypesPotedStriped = true;
                    }
                }
            ...
            if (transform.tag.Equals("WhiteBall")) {
                ...
                PoolGameManager.Instance.faultMessage = PoolStaticStrings.potedCueBall;
                DisableWhiteBall();
            } else {
                int ballNumber = System.Int32.Parse(transform.tag.Replace("Ball", ""));
                if (PoolGameManager.Instance.cueController.isServer) {
                        ...
                        if ((PoolGameManager.Instance.ownSolids && PoolGameManager.Instance.solidPoted >= 7) ||
                            ...
                            if (ballNumber == 8) {
                                if (PoolGameManager.Instance.callPocketBlack) {
                                    if (other.tag.Equals("Pot" + PoolGameManager.Instance.calledPocketID)) {
                            ...
                            if (PoolGameManager.Instance.ownSolids) {
                                if (ballNumber < 8) {
                                    if (PoolGameManager.Instance.callPocketAll) {
                                        if (other.tag.Equals("Pot" + PoolGameManager.Instance.calledPocketID)) {
                                            PoolGameManager.Instance.validPotsCount++;
                                            PoolGameManager.Instance.validPot = true;
                                        }
                                    } else {
                                        PoolGameManager.Instance.validPotsCount++;
                                        PoolGameManager.Instance.validPot = true;
                ...
                if (ballNumber < 8) {
                    PoolGameManager.Instance.solidPoted++;
                } else if (ballNumber > 8) {
                    PoolGameManager.Instance.stripedPoted++;
                }
    ...
    void OnCollisionEnter(Collision collision) {
        ...
        if (PoolGameManager.Instance.firstBallTouched && transform.tag.Contains("Ball") && collision.collider.tag.Equals("bumper")) {
            PoolGameManager.Instance.ballTouchedBand++;
        }
        ...
        if (transform.tag.Equals("WhiteBall")) {
            if (collision.collider.tag.Contains("Ball") && !collision.collider.tag.Equals("WhiteBall") && PoolGameManager.Instance.cueController.isServer) {
                // Check if touched my ball - otherwise fault
                if (!PoolGameManager.Instance.firstBallTouched) {
                    PoolGameManager.Instance.firstBallTouched = true;
                    ...
                    if (PoolGameManager.Instance.playersHaveTypes) {
                        ...
                        if ((PoolGameManager.Instance.ownSolids && PoolGameManager.Instance.solidPoted >= 7) ||
                            ...
                            if (ballNumber != 8) {
                                PoolGameManager.Instance.wasFault = true;
                                PoolGameManager.Instance.faultMessage = PoolStaticStrings.failedToHit8Ball;
                            }
                        } else {
                            if (PoolGameManager.Instance.ownSolids) {
                                if (ballNumber >= 8) {
                                    ...
                                    PoolGameManager.Instance.wasFault = true;
                                    PoolGameManager.Instance.faultMessage = PoolStaticStrings.failedToHitSolid;
                                }
                            } else {
                                if (ballNumber <= 8) {
                                    // Touched not mine ball first - fault
                                    PoolGameManager.Instance.wasFault = true;
                                    PoolGameManager.Instance.faultMessage = PoolStaticStrings.failedToHitStriped;
                                }
                            }
                        }
                    }
            ...
            if (transform.tag.Contains("Ball") && collision.collider.tag.Equals("bumper")) {
                if (!PoolGameManager.Instance.ballTouchBeforeStrike.Contains(transform.tag))
                    PoolGameManager.Instance.ballTouchBeforeStrike.Add(transform.tag);
            }
        }
    }
    ...
    public void showMessage() {
        ...
        float timeDiff = Time.time - PoolGameManager.Instance.messageTime;
        ...
        if (timeDiff > 2) {
            movePosition();
            PoolGameManager.Instance.messageTime = Time.time;
        } else {
