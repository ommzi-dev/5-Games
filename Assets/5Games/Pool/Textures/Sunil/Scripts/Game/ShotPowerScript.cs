...
public class ShotPowerScript : MonoBehaviour {
    ...
    void OnMouseUp() {
        ...
        if (!PoolGameManager.Instance.stopTimer && cueScript.isServer) {
    ...
    void Update() {
        if (mouseDown && !deactivateDone && PoolGameManager.Instance.cueController.isServer) {
