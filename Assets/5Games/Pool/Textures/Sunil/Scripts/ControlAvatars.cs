...
public class ControlAvatars : MonoBehaviour
{
    ...
    void Start()
    {
        PoolGameManager.Instance.controlAvatars = this;
        speed1 = speed;
    ...
    public void reset()
    {
        ...
        PhotonNetwork.BackgroundTimeout = PoolStaticStrings.photonDisconnectTimeout;
        ...
        PoolGameManager.Instance.readyToAnimateCoins = false;
            ...
            if (foundPlayer)
            {
                ...
                if (speed1 < speed * (2.8f / 4.0f) && !changedAvatar)
                {
                    ...
                    if (PoolGameManager.Instance.avatarOpponent != null)
                        lastAvatar.GetComponent<Image>().sprite = PoolGameManager.Instance.avatarOpponent;
                    else
                ...
                if (OpponentAvatar != null && OpponentAvatar.GetComponent<RectTransform>().anchoredPosition.y <= 0)
                {
                    ...
                    opponentNameText.text = PoolGameManager.Instance.nameOpponent;
                    ...
                    {
                        //Debug.Log("Player issue " + playerRejected);
                        playerDisconnected();
                    }
                    else
                    {
                        ...
                        StartCoroutine(countDownCoins(PoolGameManager.Instance.payoutCoins));
                    }
                    PoolGameManager.Instance.readyToAnimateCoins = true;
    ...
    private void coinsAnimate()
    {
        ...
        if (PoolGameManager.Instance.payoutCoins >= 1000)
        {
            if (PoolGameManager.Instance.payoutCoins >= 1000000)
            {
                if (PoolGameManager.Instance.payoutCoins % 1000000.0f == 0)
                {
                    my.text = (PoolGameManager.Instance.payoutCoins / 1000000.0f).ToString("0") + "M";
                    oppontent.text = (PoolGameManager.Instance.payoutCoins / 1000000.0f).ToString("0") + "M";
                }
                else
                {
                    my.text = (PoolGameManager.Instance.payoutCoins / 1000000.0f).ToString("0.0") + "M";
                    oppontent.text = (PoolGameManager.Instance.payoutCoins / 1000000.0f).ToString("0.0") + "M";
                }

            }
            else
            {
                if (PoolGameManager.Instance.payoutCoins % 1000.0f == 0)
                {
                    my.text = (PoolGameManager.Instance.payoutCoins / 1000.0f).ToString("0") + "k";
                    oppontent.text = (PoolGameManager.Instance.payoutCoins / 1000.0f).ToString("0") + "k";
                }
                else
                {
                    my.text = (PoolGameManager.Instance.payoutCoins / 1000.0f).ToString("0.0") + "k";
                    oppontent.text = (PoolGameManager.Instance.payoutCoins / 1000.0f).ToString("0.0") + "k";
                }
        ...
        {
            oppontent.text = PoolGameManager.Instance.payoutCoins + "";
            my.text = PoolGameManager.Instance.payoutCoins + "";
        }
    ...
    {
        GameObject.Find("PlayFabManager").GetComponent<PoolPlayFabManager>().imReady = true;
        if (!PoolGameManager.Instance.offlineMode)
        {

            PhotonNetwork.RaiseEvent(199, PoolGameManager.Instance.cueIndex + "-" + PoolGameManager.Instance.cueTime, true, null);
        }
    ...
    public IEnumerator updateMessageBubbleText()
    {
        ...
        if (!PoolGameManager.Instance.opponentDisconnected)
            messageBubbleText.GetComponent<Text>().text = PoolStaticStrings.waitingForOpponent + " " + waitingOpponentTime;
        if (waitingOpponentTime > 0 && !opponentActive && !PoolGameManager.Instance.opponentDisconnected)
        {
    ...
    public void cancelGameInvoke()
    {
        ...
        if (!foundPlayer && PhotonNetwork.otherPlayers.Length == 0)
        {
            ...
            PhotonNetwork.BackgroundTimeout = 0;
            Debug.Log("Timeout 2");
            PoolGameManager.Instance.playfabManager.imReady = false;
