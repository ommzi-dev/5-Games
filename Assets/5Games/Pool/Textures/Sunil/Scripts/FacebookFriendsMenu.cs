...
public class FacebookFriendsMenu : MonoBehaviour
{
    ...
    public void showFriends(List<string> friendsNames, List<string> friendsIDs, List<string> friendsAvatars)
    {
        ...
        if (friendsNames != null)
        {
            for (int i = 0; i < friendsNames.Count; i++)
            {
                ...
                friendsObjects.Add(friend);
                Debug.Log("KUPA");
                for (int j = 0; j < PoolGameManager.Instance.friendsStatuses.Count; j++)
                {

                    string[] friend1 = PoolGameManager.Instance.friendsStatuses[j];
                    Debug.Log(friendID + "  " + friend1[0]);
                    if (friend1[0].Equals(friendID))
                    {
                        Debug.Log("Found FRIEND");
                        if (friend1[1].Equals("" + ChatUserStatus.Online))
                            PoolGameManager.Instance.facebookFriendsMenu.updateFriendStatus(ChatUserStatus.Online, friendID);
                        break;
                    }
                }
    ...
    public void InviteFriend(string i)
    {
        ...
        to.Add(i);
        FB.AppRequest(
            PoolStaticStrings.facebookInviteMessage, to, null, null, null, null, null,
            delegate (IAppRequestResult result)
            {
                ...
                {
                    PoolGameManager.Instance.playfabManager.addCoinsRequest(PoolStaticStrings.rewardCoinsForFriendInvite);
                }
    ...
    {
        PoolGameManager.Instance.facebookFriendsMenu.hideFriends();
        PoolGameManager.Instance.inviteFriendActivated = true;
        PoolGameManager.Instance.challengedFriendID = id;
        PoolGameManager.Instance.initMenuScript.showSelectTableScene(true);
