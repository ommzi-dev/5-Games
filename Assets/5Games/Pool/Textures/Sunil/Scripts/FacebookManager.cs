...
public class FacebookManager : MonoBehaviour {
    ...
    private PoolPlayFabManager playFabManager;
    public string name;
    ...
    void Start() {
        ...
        facebookFriendsMenu = PoolGameManager.Instance.facebookFriendsMenu; //.GetComponent <FacebookFriendsMenu> ();
    }
    ...
    public void startRandomGame() {
        ...
        var test2 = PoolGameManager.Instance.matchPlayerObject;
        var test = PoolGameManager.Instance.matchPlayerObject.GetComponent<SetMyData>();
        PoolGameManager.Instance.matchPlayerObject.GetComponent<SetMyData>().MatchPlayer();
        PoolGameManager.Instance.matchPlayerObject.GetComponent<SetMyData>().setBackButton(true);
    ...
    private void AuthCallback(ILoginResult result) {
        if (FB.IsLoggedIn) {
            ...
            PoolGameManager.Instance.facebookIDMy = aToken.UserId;
    ...
    private void initSession() {
        ...
        if (logType.Equals("Facebook")) {
            PoolGameManager.Instance.facebookIDMy = Facebook.Unity.AccessToken.CurrentAccessToken.UserId;
            callApiToGetName();
            getMyProfilePicture(PoolGameManager.Instance.facebookIDMy);
    ...
    private void callApiToGetName() {
        FB.API("me?fields=first_name", Facebook.Unity.HttpMethod.GET, APICallbackName);
    }


    void APICallbackName(IResult response) {
        PoolGameManager.Instance.nameMy = response.ResultDictionary["first_name"].ToString();
        Debug.Log("My name " + PoolGameManager.Instance.nameMy);
    }
    ...
    public void getMyProfilePicture(string userID) {
        ...
        FB.API("/me?fields=picture.width(200).height(200)", Facebook.Unity.HttpMethod.GET, delegate (IGraphResult result) {
            if (result.Error == null) {
                ...
                PoolGameManager.Instance.avatarMyUrl = ((reqResult["picture"] as Dictionary<string, object>)["data"] as Dictionary<string, object>)["url"] as string;
                Debug.Log("My avatar " + PoolGameManager.Instance.avatarMyUrl);

                StartCoroutine(loadImageMy(PoolGameManager.Instance.avatarMyUrl));
    ...
    public IEnumerator loadImageMy(string url) {
        ...
        PoolGameManager.Instance.avatarMy = Sprite.Create(www.texture, new Rect(0, 0, www.texture.width, www.texture.height), new Vector2(0.5f, 0.5f), 32);
    ...
    public void getOpponentProfilePicture(string userID) {
        ...
        FB.API("/" + userID + "/picture?type=square&height=92&width=92", Facebook.Unity.HttpMethod.GET, delegate (IGraphResult result) {
            if (result.Texture != null) {
                ...
                PoolGameManager.Instance.avatarMy = Sprite.Create(result.Texture, new Rect(0, 0, result.Texture.width, result.Texture.height), new Vector2(0.5f, 0.5f), 32);
            ...
            FB.API("/me/invitable_friends?limit=5000&fields=id,name,picture.width(100).height(100)", Facebook.Unity.HttpMethod.GET, delegate (IGraphResult result) {
                ...
                if (result.Error == null) {
                    ...
                    for (int xx = 0; xx < newObj.Count; xx++) {
                        ...
                        PoolGameManager.Instance.facebookFriendsMenu.AddFacebookFriend(name, id, avatarUrl);
                    }
