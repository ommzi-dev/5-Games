...
public class DiceMultiplayer : Photon.PunBehaviour, IChatClientListener, IPunObservable
{
    
    public static DiceMultiplayer Instance = null;
    ...
    public string MatchMakingString = "dice";
   ...
   // public Text PlayingPlayerText;
    public string table;
    public Text opponentNameonBoard;
    ...
    public Text playernameOnSearch;
    ...
    void Start()
    {
        playernameOnSearch.text= UserDetailsManager.userName;
        GetPhotonToken();
        turnof = "player";
    ...
    void Update()
    {
        ...
        if (DiceGameManager.instance.isMultiplayer)
        {
            if(DiceGameManager.instance.TryAgainButton.activeInHierarchy)
            {
                return;
            }
            if (!DiceCheckZoneScript.instance.isCoroutineStarted)
            {
                if (DiceGameManager.instance.multiplayerPlayerCount > 0 && DiceGameManager.instance.cpuCount > 0)
                {

                    if (DiceGameManager.instance.multiplayerPlayerCount > DiceGameManager.instance.cpuCount)
                    {
                        StartCoroutine(DiceCheckZoneScript.instance.OnPopUp("win"));
                    ...
                    if (DiceGameManager.instance.multiplayerPlayerCount == DiceGameManager.instance.cpuCount)
                    {
                        StartCoroutine(DiceCheckZoneScript.instance.OnPopUp("draw"));
                    }
                }
            }
    ...
    void CheckIfRoomAvailable()
    {
        ...
        foreach (RoomInfo temp in PhotonNetwork.GetRoomList())
        {
            ...
            { 
                if (expectedCustomRoomProperties["game"].ToString() == "dice" && !bool.Parse(expectedCustomRoomProperties["isChallenge"].ToString()))
                {
    ...
    public override void OnReceivedRoomListUpdate()
    {
        ...
        foreach (RoomInfo temp in PhotonNetwork.GetRoomList())
        {
            ...
            if (expectedCustomRoomProperties["game"].ToString() == "dice" && expectedCustomRoomProperties["table"].ToString()==table && !bool.Parse(expectedCustomRoomProperties["isChallenge"].ToString()) && temp.PlayerCount < 2)
            {
                ...
                roomInfo.GetComponent<DiceRoomDetails>().playerId = expectedCustomRoomProperties["ownerid"].ToString();
                roomInfo.GetComponent<DiceRoomDetails>().playerName = expectedCustomRoomProperties["ownername"].ToString();
                roomInfo.GetComponent<DiceRoomDetails>().poolId = expectedCustomRoomProperties["poolId"].ToString();
                roomInfo.GetComponent<DiceRoomDetails>().betAmount = int.Parse(expectedCustomRoomProperties["bet"].ToString());
                roomInfo.GetComponent<DiceRoomDetails>().winningAmount = int.Parse(expectedCustomRoomProperties["winAmt"].ToString());
                roomInfo.GetComponent<DiceRoomDetails>().SetDetails();
    ...
    public IEnumerator TransactionPool(string winner, string looser)
    {
        ...
        form.AddField("poolid", DiceMultiplayer.Instance.poolId);
    ...
    {
        yield return new WaitForSecondsRealtime(1f*2.5f);
        disconnectTimer -= 1;
        ...
        if (PhotonNetwork.insideLobby)
        {
          ...
          //  p.avatarCode = 1;
            ...
            if (gameStarted)
            {
                ...
                roomOptions.CustomRoomProperties = new ExitGames.Client.Photon.Hashtable() { { "ownername", UserDetailsManager.userName }, { "ownerid", UserDetailsManager.userId }, { "bet", betAmount }, { "isAvailable", true }, { "appVer", Application.version }, { "poolId", poolId }, { "isChallenge", isChallenge }, { "winAmt", winAmt}, { "game", "dice" },{"table",table } };
...
//        PlayingPlayerText.text = UserDetailsManager.userName;
        ...
        {
            DiceMultiplayer.Instance.myTurn = 0;
            myTurn = 0;
        }
        else
        {
        ...
        DiceGameManager.instance.OnPressPractice();
        SearchOpponentPanel.SetActive(false);
    ...
    [PunRPC]
    void OnPlayerScoreDice(int score)
    {
        Debug.Log("Player send score received " + score);

        DiceGameManager.instance.cpuCount = score;
        DiceGameManager.instance.cpuText.text = score.ToString();
